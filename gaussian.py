...
class FullGaussian2d(Readout):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print("jnfsdkjsdnkjsdnkj")
        self.Model1 = nn.Sequential(
            nn.Linear(outdims*self.no_of_samples, outdims*int(self.no_of_samples/2)),
            nn.ReLU(),
            nn.Linear(outdims*int(self.no_of_samples/2), outdims),
            )
        print("this is done")
        self.feature_reg_weight = self.resolve_deprecated_gamma_readout(feature_reg_weight, gamma_readout)
        self.mean_activity = mean_activity
    ...
    def forward(self, x, sample=None, shift=None, out_idx=None, **kwargs):
        ...
        self.new_y = torch.tensor([[0]]).to('cuda:0')

        for i in range (n):
            y = F.grid_sample(x, grid, align_corners=self.align_corners)
            y = (y.squeeze(-1) * feat).sum(1).view(N, outdims).to('cuda:0')
            self.new_y = torch.cat((self.new_y, y), dim = 1)
        
        y = torch.squeeze(y)
        y = torch.tensor(self.new_y)
        y = self.Model1(y)
